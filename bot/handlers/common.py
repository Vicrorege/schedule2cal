...
5. Проверь превью → ✅ Подтвердить → запись в календарь
...
• После записи в календарь пароль стирается из бота
...
def _start_text(*, class_name: str | None = None, subgroup: int | None = None) -> str:
    text = (
        ...
        "Выбери действие в меню ниже или просто пришли PDF/фото."
    )
    if class_name:
